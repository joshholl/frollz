      ...
      <form @submit.prevent="createFormat">
        <div class="mb-4">
          <label
            ...
            >Package <span class="text-red-500" aria-hidden="true">*</span>
            <select
              id="format-package"
              v-model="newFormat.packageId"
              required
              aria-required="true"
        ...
        <div class="mb-4">
          <label
            for="format-name"
            class="block text-sm font-medium text-gray-700 dark:text-gray-300"
            >Format Name <span class="text-red-500" aria-hidden="true">*</span>
            <input
              id="format-name"
...
const createFormat = async () => {
  ...
  try {
    await formatApi.create({
      ...newFormat.value,
      packageId: Number(newFormat.value.packageId),
    });
