...
import { filmApi, emulsionApi, transitionApi, formatApi, tagApi, cameraApi, } from "@/services/api-client";
...
describe("FilmsView", () => {
    beforeEach(() => {
        ...
        vi.clearAllMocks();
        vi.mocked(filmApi.getAll).mockResolvedValue({ data: [] });
        vi.mocked(emulsionApi.getAll).mockResolvedValue({ data: [] });
        vi.mocked(formatApi.getAll).mockResolvedValue({ data: [] });
    ...
    describe("component mounting", () => {
        ...
        it("should display films after loading", async () => {
            ...
            vi.mocked(filmApi.getAll).mockResolvedValue({ data: [film] });
            const wrapper = mount(FilmsView, { global: { plugins: [router] } });
            await flushPromises();
    ...
    describe("search", () => {
        ...
        it("should include search chip in activeFilterChips when searchQuery is set", async () => {
            ...
            vm.searchQuery = "scotland";
            await wrapper.vm.$nextTick();
            const chip = vm.activeFilterChips.find((c) => c.key === "search");
    ...
    describe("scan link indicator", () => {
        const makeFilmWithScans = (urls) => makeFilm({
            states: [
                {
                    id: 1,
                    filmId: 1,
        ...
        it("renders an external link for a film with 1 scan URL", async () => {
            ...
            const externalLink = indicators.find((el) => el.element.tagName === "A" &&
        ...
        it("links to film detail for a film with multiple scan URLs (desktop)", async () => {
            ...
            const indicators = wrapper.findAll('[data-testid="scan-indicator"]');
            const detailLink = indicators.find((el) => el.element.tagName === "A" &&
    ...
    describe("add film form", () => {
        ...
        it("should set bulk profile when bulk roll emulsion is selected", async () => {
            ...
            const bulkEmulsion = vm.emulsions.find((e) => e.formatId === 3);
            if (bulkEmulsion) {
        ...
        it("should set standard profile when non-bulk emulsion is selected", async () => {
            ...
            const standardEmulsion = vm.emulsions.find((e) => e.formatId !== 3);
            if (standardEmulsion) {
        ...
        it("should toggle bulk profile when user checks create as bulk for roll format", async () => {
            ...
            const rollEmulsion = vm.emulsions.find((e) => {
                const fmt = vm.formats.find((f) => f.id === e.formatId);
