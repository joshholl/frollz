...
describe("FilmFormatsView", () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(formatApi.create).mockResolvedValue({
            data: mockFormats[0],
        });
        vi.mocked(formatApi.delete).mockResolvedValue({});
        vi.mocked(packageApi.getAll).mockResolvedValue({
            data: mockPackages,
        });
    });
    ...
    describe("create format", () => {
        ...
        it("should call formatApi.create and reload on form submit", async () => {
            ...
            await flushPromises();
            expect(formatApi.create).toHaveBeenCalledWith({
                packageId: 1,
                name: "35mm",
            });
