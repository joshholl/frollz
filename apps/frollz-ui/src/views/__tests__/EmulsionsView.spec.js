...
describe("StocksView", () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(emulsionApi.getAll).mockResolvedValue({
            data: mockEmulsions,
        });
        vi.mocked(formatApi.getAll).mockResolvedValue({ data: mockFormats });
        vi.mocked(processApi.getAll).mockResolvedValue({
            data: mockProcesses,
        });
        vi.mocked(tagApi.getAll).mockResolvedValue({ data: [] });
        vi.mocked(emulsionApi.addTag).mockResolvedValue({ data: {} });
        vi.mocked(emulsionApi.getBrands).mockResolvedValue({ data: [] });
        vi.mocked(emulsionApi.getManufacturers).mockResolvedValue({
            data: [],
        });
    });
    afterEach(() => {
    ...
    describe("form validation", () => {
        ...
        it("should proceed with API call when formats are selected", async () => {
            ...
            vi.mocked(emulsionApi.createBulk).mockResolvedValue({
                data: mockCreated,
            });
            const wrapper = mount(EmulsionsView);
            const vm = wrapper.vm;
    ...
    describe("sorting", () => {
        ...
        it("should default sort by brand ascending", async () => {
            ...
            const brands = vm.sortedEmulsions.map((s) => s.brand);
        ...
        it("should sort by a different field when setSort is called", async () => {
            ...
            expect(vm.sortDirection).toBe("asc");
            const manufacturers = vm.sortedEmulsions.map((s) => s.manufacturer);
        ...
        it("should reset to ascending when switching to a new field", async () => {
            ...
            expect(vm.sortDirection).toBe("asc");
            const speeds = vm.sortedEmulsions.map((s) => s.speed);
        ...
        it("should sort numerically by speed", async () => {
            ...
            vm.setSort("speed");
            await wrapper.vm.$nextTick();
            expect(vm.sortedEmulsions.map((s) => s.speed)).toEqual([
                100, 200, 400,
            ]);
    ...
    describe("filtering", () => {
        ...
        it("should add a filter via addFilter", async () => {
            ...
            expect(vm.activeFilters[0]).toEqual({
                field: "manufacturer",
                label: "Manufacturer",
                value: "Kodak",
            });
