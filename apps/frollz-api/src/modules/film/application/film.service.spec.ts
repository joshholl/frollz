...
const makeFilmState = (
  overrides: Partial<Parameters<typeof FilmState.create>[0]> = {},
): FilmState =>
  FilmState.create({
    id: randomId(),
    filmId: randomId(),
...
const makeTransitionState = (
  overrides: Partial<Parameters<typeof TransitionState.create>[0]> = {},
): TransitionState =>
  TransitionState.create({ id: randomId(), name: "Added", ...overrides });

const makeTransitionRule = (
  overrides: Partial<Parameters<typeof TransitionRule.create>[0]> = {},
): TransitionRule =>
...
describe("FilmService", () => {
  describe("findAll", () => {
    ...
    it("returns empty array when state name does not match any known state", async () => {
      ...
      await expect(
        service.findAll({ stateNames: ["Unknown"] }),
      ).resolves.toEqual([]);
    });
  ...
  describe("create", () => {
    it("saves the film and inserts an initial Added state", async () => {
      ...
      expect(filmRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ name: "Roll 001" }),
      );
  ...
  describe("transition", () => {
    it("records a new film state when the transition is valid", async () => {
      ...
      const rule = makeTransitionRule({
        fromStateId: addedStateId,
        toStateId: loadedState.id,
      });
    ...
    it("throws NotFoundException when target state does not exist", async () => {
      ...
      await expect(
        service.transition(film.id, { targetStateName: "Nonexistent" }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it("throws BadRequestException when the transition is not permitted", async () => {
      ...
      await expect(
        service.transition(film.id, { targetStateName: "Received" }),
