...
export interface ICameraRepository {
  findAll(criteria: { brand?: string; model?: string; status?: CameraStatus; formatId?: number; unloaded?: boolean; }): Camera[] | PromiseLike<Camera[]>;
  findById(id: number): Promise<Camera | null>;
  save(camera: Camera): Promise<number>;
