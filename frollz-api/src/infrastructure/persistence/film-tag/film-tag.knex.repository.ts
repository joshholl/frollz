...
@Injectable()
export class FilmTagKnexRepository  extends BaseKnexRepository  implements IFilmTagRepository {
  ...
  async remove(filmId: number, tagId: number): Promise<void> {
    await this.db('film_tag').where({ film_id: filmId, tag_id: tagId }).delete();
  }
}
